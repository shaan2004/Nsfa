...
const GoldText = ({ text, className = "" }: { text: string; className?: string }) => (
  <h2
    ...
    style={{
      // Keeping the text gradient slightly more contrasted for readability against dark backgrounds
...
// Premium Metallic Gold Gradient (Matched to your reference image)
  ...
  return (
    <main suppressHydrationWarning className="bg-[#040814] text-white min-h-screen pt-32 pb-32 relative">
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        >
          {categories.map((category) => (
            <button
              ...
              className={`px-8 py-3 rounded-2xl text-sm font-bold tracking-widest uppercase transition-all duration-300 ${
        ...
        {/* 3. ROYAL ART GALLERY GRID (Optimized & Premium Gold Hover) */}
          ...
          <AnimatePresence mode="wait">
            {filteredEvents.map((event, index) => (
              ...
              >
                ...
                <div className="p-2 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.8)] transition-all duration-500 group-hover:shadow-[0_20px_50px_rgba(212,175,55,0.25)] group-hover:-translate-y-2 flex flex-col h-full relative overflow-hidden">
                  
                  {/* --- DUAL LAYER BACKGROUND FOR SMOOTH CROSSFADE --- */}
                  <div className="absolute inset-0 bg-[#0A1128] border-2 border-[#BF953F]/30 rounded-[2rem] transition-opacity duration-500 z-0 group-hover:opacity-0" />
                  <div className={`absolute inset-0 ${premiumGoldGradient} rounded-[2rem] opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-0`} />

                  {/* Inner Matte & Image */}
                  <div className="relative overflow-hidden aspect-[4/3] bg-black rounded-xl m-2 z-10 border border-white/10 group-hover:border-[#040814]/20 transition-colors duration-500">
                    <Image
                      src={event.cover}
                      alt={event.title}
                      fill
                    ...
                    <div className="absolute top-4 right-4 bg-[#0A1128]/80 backdrop-blur-md border border-[#BF953F]/50 px-3 py-1.5 rounded-xl flex items-center gap-2 shadow-lg group-hover:bg-[#040814] transition-colors duration-500">
                      <Images size={14} className="text-[#FBF5B7]" />
                      <span className="text-[#FBF5B7] text-xs font-bold tracking-wider">{event.images.length}</span>
                    </div>
                  </div>

                  {/* Card Info Area - Text inverts on hover to contrast with the new gold background */}
                  <div className="p-6 pb-4 flex flex-col items-center justify-center text-center flex-grow relative z-10">
                    <span className="text-[#BF953F] group-hover:text-[#040814]/70 transition-colors duration-500 text-xs font-bold uppercase tracking-[0.2em] mb-3">
                      {event.category}
                    </span>
                    <h4 className="text-white group-hover:text-[#040814] transition-colors duration-500 font-serif text-xl font-bold mb-6 leading-snug drop-shadow-md group-hover:drop-shadow-none">
                      {event.title}
                    </h4>
                    
                   {/* View Button - Fixed Hover State */}
                    <button className="px-8 py-3 rounded-xl border border-[#BF953F]/40 bg-[linear-gradient(180deg,#121F3D,#0A1128)] group-hover:bg-none group-hover:bg-[#040814] text-[#FBF5B7] group-hover:text-[#FFF2CD] group-hover:border-[#040814] font-bold transition-all duration-500 text-xs uppercase tracking-widest shadow-inner mt-auto w-full">
                      View Gallery
                    </button>
                  </div>
        ...
        {selectedEvent && (
          ...
          >
            ...
            <motion.div 
              ...
              className="absolute top-0 left-0 w-full p-6 flex justify-between items-center z-[60] bg-gradient-to-b from-black/80 to-transparent"
            >
               <div className="text-white pl-2 md:pl-6">
                 <h3 className="font-serif text-xl md:text-3xl text-[#FBF5B7] drop-shadow-lg">{selectedEvent.title}</h3>
                 <p className="text-white/60 text-xs md:text-sm tracking-widest uppercase mt-2">Photo {currentImageIndex + 1} of {selectedEvent.images.length}</p>
               </div>
               
               <button 
                onClick={closeLightbox}
                className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-white/5 border border-white/20 flex items-center justify-center text-white/90 hover:text-[#040814] hover:bg-[linear-gradient(145deg,#D4AF37_0%,#FFF2CD_45%,#AA771C_100%)] hover:border-transparent transition-all shadow-lg"
               >
                 <X size={28} />
               </button>
            </motion.div>
           ...
           {selectedEvent.images.length > 1 && (
              <>
                <button 
                  ...
                  className="absolute left-4 md:left-10 top-1/2 -translate-y-1/2 w-12 h-12 md:w-16 md:h-16 rounded-full bg-[#0A1128]/80 border border-[#BF953F]/40 flex items-center justify-center text-[#FBF5B7] hover:bg-[linear-gradient(145deg,#D4AF37_0%,#FFF2CD_45%,#AA771C_100%)] hover:text-[#040814] hover:border-transparent transition-all z-[60] shadow-xl"
                >
                  <ChevronLeft size={32} />
                </button>
                ...
                <button 
                  ...
                  className="absolute right-4 md:right-10 top-1/2 -translate-y-1/2 w-12 h-12 md:w-16 md:h-16 rounded-full bg-[#0A1128]/80 border border-[#BF953F]/40 flex items-center justify-center text-[#FBF5B7] hover:bg-[linear-gradient(145deg,#D4AF37_0%,#FFF2CD_45%,#AA771C_100%)] hover:text-[#040814] hover:border-transparent transition-all z-[60] shadow-xl"
                >
            ...
            {/* The Image Container */}
            <div className="relative w-full h-[80vh] md:h-[85vh] flex items-center justify-center px-12 md:px-32" onClick={(e) => e.stopPropagation()}>
               <Image
            ...
            {selectedEvent.images.length > 1 && (
              <motion.div 
                ...
                className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-3 z-[60] bg-[#0A1128]/80 p-3 rounded-2xl border border-[#BF953F]/30 max-w-[90vw] overflow-x-auto shadow-2xl backdrop-blur-md" 
              ...
              >
                {selectedEvent.images.map((imgSrc, idx) => (
                  <button 
                    ...
                    className={`relative w-16 h-12 md:w-20 md:h-14 shrink-0 rounded-lg overflow-hidden transition-all border-2 ${currentImageIndex === idx ? "border-[#FBF5B7] opacity-100 scale-105" : "border-transparent opacity-40 hover:opacity-100"}`}
                  >
